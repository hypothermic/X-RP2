...
public class ContainerEject
        extends Container {
    private TileEjectBase tileEject;

    public ContainerEject(IInventory iInventory, TileEjectBase tileEjectBase) {
        int n;
        int n2;
        this.tileEject = tileEjectBase;
        for (n2 = 0; n2 < 3; ++n2) {
            for (n = 0; n < 3; ++n) {
                this.a(new Slot((IInventory) tileEjectBase, n + n2 * 3, 62 + n * 18, 17 + n2 * 18));
            }
        }
        for (n2 = 0; n2 < 3; ++n2) {
            for (n = 0; n < 9; ++n) {
    ...
    public IInventory getInventory() {
        return this.tileEject;
    }

    public boolean b(EntityHuman entityHuman) {
        return this.tileEject.a(entityHuman);
    }
